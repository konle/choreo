...
mod tests {
    ...
    impl PluginExecutor for StubExecutor {
        async fn execute_node_instance(
            &self, _ni: &mut WorkflowNodeInstanceEntity, _wi: &mut WorkflowInstanceEntity,
        ) -> anyhow::Result<ExecutionResult> {
            unreachable!()
        }
        async fn handle_node_callback(
            &self, _ni: &mut WorkflowNodeInstanceEntity, _wi: &mut WorkflowInstanceEntity,
            _cid: &str, _st: &NodeExecutionStatus, _out: &Option<serde_json::Value>,
            _err: &Option<String>, _inp: &Option<serde_json::Value>,
        ) -> anyhow::Result<ExecutionResult> {
            unreachable!()
        }
        async fn resolve_child_status(
            &self, _child_task_instance_id: &str, _task_template: &TaskTemplate,
        ) -> crate::plugin::interface::ChildStatus {
            unreachable!()
        }
    }

    fn make_node(plugin: &HttpPlugin, wf: &WorkflowInstanceEntity, node_id: &str) -> WorkflowNodeInstanceEntity {
        let now = Utc::now();
        WorkflowNodeInstanceEntity {
            ...
            task_instance: crate::task::entity::task_definition::TaskInstanceEntity {
                ...
                task_template: TaskTemplate::Http(crate::task::entity::task_definition::TaskHttpTemplate {
                    url: "/test".into(), method: crate::task::entity::task_definition::HttpMethod::Get,
    ...
    fn make_instance() -> WorkflowInstanceEntity {
        ...
        WorkflowInstanceEntity {
            ...
            created_at: now, updated_at: now, deleted_at: None,
            context: serde_json::json!({}),
            entry_node: "h1".into(), current_node: "h1".into(),
            nodes: vec![], epoch: 0,
    ...
    async fn handle_callback_success_copies_output() {
        ...
        let result = plugin.handle_callback(
            &StubExecutor, &mut node, &mut wf, "wf1-h1",
            &NodeExecutionStatus::Success, &Some(output.clone()), &None, &Some(input.clone()),
        ).await.unwrap();
    ...
    async fn handle_callback_failed_sets_error() {
        ...
        let result = plugin.handle_callback(
            &StubExecutor, &mut node, &mut wf, "wf1-h1",
            &NodeExecutionStatus::Failed, &None, &Some(err.clone()), &None,
        ).await.unwrap();
