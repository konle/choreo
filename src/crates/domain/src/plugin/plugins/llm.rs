...
mod tests {
    ...
    fn make_node(plugin: &LlmPlugin, wf: &WorkflowInstanceEntity, node_id: &str) -> WorkflowNodeInstanceEntity {
        let now = Utc::now();
        WorkflowNodeInstanceEntity {
            ...
            task_instance: crate::task::entity::task_definition::TaskInstanceEntity {
                ...
                task_template: TaskTemplate::Llm(crate::task::entity::task_definition::LlmTemplate {
    ...
    fn make_instance() -> WorkflowInstanceEntity {
        ...
        WorkflowInstanceEntity {
            ...
            created_at: now, updated_at: now, deleted_at: None,
            context: serde_json::json!({}),
            entry_node: "llm1".into(), current_node: "llm1".into(),
            nodes: vec![], epoch: 0,
    ...
    async fn handle_callback_success_copies_output() {
        ...
        let result = plugin.handle_callback(
            &StubExecutor, &mut node, &mut wf, "wf1-llm1",
            &NodeExecutionStatus::Success, &Some(output.clone()), &None, &None,
        ).await.unwrap();
    ...
    async fn handle_callback_failed_sets_error() {
        ...
        let result = plugin.handle_callback(
            &StubExecutor, &mut node, &mut wf, "wf1-llm1",
            &NodeExecutionStatus::Failed, &None, &Some("rate limit".into()), &None,
        ).await.unwrap();
