...
impl PluginInterface for IfConditionPlugin {
    async fn execute(
        ...
        let template = match &node_instance.task_instance.task_template {
            ...
            other => {
                ...
                return Err(anyhow::anyhow!(
                    "Invalid task template for IfConditionPlugin"
                ));
            }
        };
        ...
        let result: bool = engine
            .eval_with_scope(&mut scope, &template.condition)
            .map_err(|e| {
                error!(
...
mod tests {
    ...
    use crate::task::entity::task_definition::{
        IfConditionTemplate, TaskInstanceEntity, TaskTemplate,
    };
    use crate::workflow::entity::workflow_definition::{
    ...
    fn make_instance() -> WorkflowInstanceEntity {
        ...
        WorkflowInstanceEntity {
            ...
            updated_at: now,
            deleted_at: None,
            context: serde_json::json!({"score": 80}),
            entry_node: "c1".into(),
            current_node: "c1".into(),
            nodes: vec![],
            epoch: 0,
            locked_by: None,
    ...
    async fn false_condition_jumps_to_else_task() {
        ...
        assert_eq!(
            node.task_instance.output.as_ref().unwrap()["if_condition_result"],
            false
        );
    }
    ...
    async fn invalid_template_returns_error() {
        ...
        let mut node = WorkflowNodeInstanceEntity {
            ...
            task_instance: TaskInstanceEntity {
                ...
                task_id: "".into(),
                task_name: "bad".into(),
                task_type: TaskType::IfCondition,
    ...
    fn plugin_type_is_ifcondition() {
        assert_eq!(
            IfConditionPlugin::new().plugin_type(),
            TaskType::IfCondition
        );
    }
}
